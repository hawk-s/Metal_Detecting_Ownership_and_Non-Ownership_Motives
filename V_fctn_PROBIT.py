...
'''
def probit_regression_roc(data, dependent_var, independent_vars):
    """
    Perform Probit regression and compute ROC curve.
...
import matplotlib.pyplot as plt
import numpy as np
from Z_fctn_WALD_TEST_JOINT import wald_test


def probit_regression_roc_wald(data, dependent_var, independent_vars, pev_vars=None, ape_vars=None, hypotheses_variables=None):
    """
    ...
    if ape_vars:
        ape_results = results.get_margeff(at='mean', method='dydx', atexog=data[ape_vars])
...
# Perform Probit regression, ROC curve, confusion matrix, PEA, APE, and Wald test
probit_results = probit_regression_roc_wald(df_log_3, dependent_var='rate_artifs_dummy', independent_vars=['log_experience', 'log_contributions'], hypotheses_variables=hypotheses_variables)
'''